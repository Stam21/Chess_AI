                ...
                if  (x - counter >= 0 and (not flagW)):
                    if (self.board[x-counter][y] == "--" or self.board[x-counter][y][0] == opponent) and self.calculateLine(x-counter,y,threats,pos_King) and (self.findPins(x,y,self.pins)) :
                            ...
                            if (self.board[x-counter][y][0] == opponent):
                                flagW = True
                    elif (self.board[x-counter][y] == "--" or self.board[x-counter][y][0] == opponent) and self.calculateLine(x-counter,y,threats,pos_King) and len(threats) != 0:
                            if (x-counter == threats[0][0] and y == threats[0][1]):
                                self.moves.append(Actions.Move((x,y),(x-counter,y),self.board))
                            if (self.board[x-counter][y][0] == opponent):
                                flagW = True
                    
                    elif ((self.board[x-counter][y] == "--" or self.board[x-counter][y][0] == opponent) and not self.findPins(x,y,self.pins) and len(threats) == 0):
                         objective = self.board[x-counter][y]
                         piece = self.board[x][y]
                         prevMoves = self.moves
                         prevPins = self.pins
                         prevThreats = self.threats
                ...
                if  (y - counter >= 0 and (not flagN)):
                    if (self.board[x][y-counter] == "--" or self.board[x][y-counter][0] == opponent) and self.calculateLine(x,y-counter,threats,pos_King) and (self.findPins(x,y,self.pins)) :
                            ...
                            if (self.board[x][y-counter][0] == opponent):
                                flagN = True
                    elif (self.board[x][y-counter] == "--" or self.board[x][y-counter][0] == opponent) and self.calculateLine(x,y-counter,threats,pos_King) and len(threats) != 0:
                            if (x == threats[0][0] and y-counter == threats[0][1]):
                                self.moves.append(Actions.Move((x,y),(x,y-counter),self.board))
                            if (self.board[x][y-counter][0] == opponent):
                                flagN = True
                    
                    elif ((self.board[x][y-counter] == "--" or self.board[x][y-counter][0] == opponent) and not self.findPins(x,y,self.pins) and len(threats) == 0):
                         objective = self.board[x][y-counter]
                         piece = self.board[x][y]
                         prevMoves = self.moves
                         prevPins = self.pins
                         prevThreats = self.threats
                ...
                if (x - counter >= 0 and y-counter >= 0 and (not flagNW)):
                    if (self.board[x-counter][y-counter] == "--" or self.board[x-counter][y-counter][0] == opponent) and self.calculateLine(x-counter,y-counter,threats,pos_King) and (self.findPins(x,y,self.pins)) :
                            ...
                            if (self.board[x-counter][y-counter][0] == opponent):
                                flagNW = True
                    elif (self.board[x-counter][y-counter] == "--" or self.board[x-counter][y-counter][0] == opponent) and self.calculateLine(x-counter,y-counter,threats,pos_King) and len(threats) != 0:
                            if (x-counter == threats[0][0] and y-counter == threats[0][1]):
                                self.moves.append(Actions.Move((x,y),(x-counter,y-counter),self.board))
                            if (self.board[x-counter][y-counter][0] == opponent):
                                    flagNW = True
    ...
    def calculateLine(self,x,y,threats,king):
        ...
        if (len(threats)==0):
            ...
            for threat in threats:
                imposibleValue = 10000000000 #when both slopes are divided by zero slope1 == slope2 but slope1 != 0 or any other posible value 
                if (threat[1]-king[1] == 0 ):
                    slope1 = imposibleValue
                else:
                    slope1 = (threat[0] - king[0]) / (threat[1] - king[1])
                if (y-king[1]== 0):
                    slope2 = imposibleValue
                else:
